...
def test_szengine_add_record_with_info_005():
    ...
    assert response.record_id == "WITH_INFO_1"
    assert response.affected_entities[0].entity_id == 7
...
def test_szengine_delete_record_with_info_002():
    ...
    assert response.data_source == "TEST"
    assert response.record_id == "DELETE_TEST"
    assert response.affected_entities[0].entity_id == 100002
